      ...
      <div className="w-full lg:w-[45%] p-8 md:p-16 flex flex-col justify-center relative bg-white z-10">
        ...
        <div className="max-w-md mx-auto w-full">
          <div className="mb-10 text-center lg:text-left">
            ...
            <h1 className="text-3xl font-display font-semibold text-[#000] mb-3">Create Your Account</h1>
            <p className="text-slate-500 font-medium text-sm">Start your digital business journey with a smarter platform built for growth.</p>
          </div>
          ...
          {authMethod === "email" ? (
            <form onSubmit={handleSignup} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <Input label="Full Name" name="name" icon={User} placeholder="John Doe" value={formData.name} onChange={handleChange} required />
                <Input label="Business Name" name="businessName" icon={Briefcase} placeholder="My Store Inc" value={formData.businessName} onChange={handleChange} required />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <Input label="Mobile Number" name="phone" type="tel" icon={Smartphone} placeholder="+91 98765 43210" value={formData.phone} onChange={handleChange} required />
              </div>
                   ...
                   {[1, 2, 3, 4].map(level => (
                     <div key={level} className={cn(
                       ...
                       passwordStrength(formData.password) >= level 
                        ? (passwordStrength(formData.password) <= 2 ? "bg-red-400" : passwordStrength(formData.password) === 3 ? "bg-yellow-400" : "bg-green-500")
                        : "bg-slate-100"
                     )} />
                   ))}
                </div>
              </div>
              <Input label="Confirm Password" name="confirmPassword" type="password" icon={Lock} placeholder="••••••••" value={formData.confirmPassword} onChange={handleChange} required />
              ...
              <div className="flex items-start gap-3 py-2">
                 <div className="relative flex items-center pt-0.5">
                  ...
                  <label htmlFor="terms" className="text-xs font-medium text-slate-500 leading-tight cursor-pointer">
                     By creating an account, you agree to our <Link to="/terms-and-conditions" className="text-[#FF6B00] hover:underline">Terms of Service</Link> and <Link to="/privacy-policy" className="text-[#FF6B00] hover:underline">Privacy Policy</Link>.
                  </label>
              </div>
              ...
              <button type="button" disabled className="w-full flex items-center justify-center gap-3 py-4 border border-slate-100 bg-slate-50/50 rounded-2xl cursor-not-allowed group opacity-70">
                ...
                <span className="text-xs font-bold text-slate-400">Sign up with Google</span>
            ...
            <div className="py-12 text-center space-y-6">
               ...
               <div className="space-y-2">
                  <h3 className="text-xl font-semibold">Mobile OTP is Coming Soon</h3>
                  <p className="text-sm text-slate-500 max-w-[280px] mx-auto">We're currently perfecting our secure SMS authentication. Please use Email Signup for now.</p>
               </div>
               <button 
                 onClick={() => setAuthMethod("email")}
          ...
          <div className="mt-10 text-center">
            <p className="text-sm font-medium text-slate-500">
              Already have an account? <Link to="/login" className="text-[#FF6B00] font-semibold hover:underline">Login Here</Link>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="relative z-10 w-full max-w-2xl">
           <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[3rem] p-12 shadow-2xl">
               <div className="flex items-center gap-4 mb-12">
                  <div className="w-14 h-14 bg-[#FF6B00] rounded-2xl flex items-center justify-center text-white shadow-xl shadow-orange-500/20">
                     <TrendingUp size={28} />
                  </div>
                 <div>
                    <h2 className="text-2xl font-display font-semibold text-white">Built for Scalable Growth</h2>
                    <p className="text-slate-400 font-medium">Trusted by 10,000+ businesses globally.</p>
                 </div>
              </div>
              
              <div className="space-y-8">
                 {[
                   { title: "Enterprise Security", desc: "Military-grade encryption for all your sensitive data." },
                   { title: "Real-time Analytics", desc: "Monitor your performance from anywhere in the world." },
                   { title: "Smart Automation", desc: "Let our AI handle the tedious work while you grow." }
                 ].map((item, i) => (
                   <div key={i} className="flex gap-5 group">
                      <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center mt-1 text-orange-400 group-hover:bg-orange-400 group-hover:text-white transition-all">
                        <Check size={14} />
                      </div>
                      <div>
              ...
              <div className="mt-16 pt-10 border-t border-white/5 flex items-center justify-between">
                 <div className="flex -space-x-3">
                    {[1,2,3,4].map(i => (
                      <img key={i} src={`https://i.pravatar.cc/100?img=${i+30}`} className="w-10 h-10 rounded-full border-2 border-[#1A1A2E]" alt="User" />
                    ))}
                    <div className="w-10 h-10 rounded-full border-2 border-[#1A1A2E] bg-orange-500 flex items-center justify-center text-[10px] font-bold text-white">+2k</div>
                 </div>
                 <p className="text-slate-400 text-xs font-semibold tracking-widest uppercase">Verified Secure</p>
              </div>
           </div>
        </div>
      </div>
