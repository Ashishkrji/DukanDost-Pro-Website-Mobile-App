...
router.get('/:id', async (req, res) => {
  try {
    const txn = await Transaction.findById(req.params.id).populate('customerId');
...
router.post('/', async (req, res) => {
  try {
    ...
    const txn = new Transaction({ customerId, type, amount, note, paymentMode });
    ...
    if (customer) {
      ...
      if (customer.balance === 0) {
        ...
        await Reminder.updateMany(
          { customerId, isActive: true, stopCondition: 'Until payment received' },
